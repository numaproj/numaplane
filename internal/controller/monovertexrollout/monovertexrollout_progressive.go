...
func (r *MonoVertexRolloutReconciler) ProcessPromotedChildPostUpgrade(
	...
	requeue, err := scaleMonoVertexToOriginalValues(ctx, rolloutPromotedChildStatus, promotedChildDef, c)
	if err != nil {
		return true, err
	}
...
func scaleMonoVertexToOriginalValues(
	...
	if rolloutPromotedChildStatus.ScaleValues[promotedChildDef.GetName()].OriginalScaleDefinition != nil {
		patchJson = fmt.Sprintf(`{"spec": {"scale": %s}}`, *rolloutPromotedChildStatus.ScaleValues[promotedChildDef.GetName()].OriginalScaleDefinition)
	}
