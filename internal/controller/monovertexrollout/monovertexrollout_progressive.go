...
func (r *MonoVertexRolloutReconciler) ProcessPromotedChildPreUpgrade(
	...
	// retrieves the currently running pods to update the promotedMVStatus scaleValues.
	// This serves to make sure that the pods have been really scaled down before proceeding
	// with the progressive upgrade.
	performedScaling, err := scaleDownMonoVertex(ctx, monoVertexRollout.Status.ProgressiveStatus.PromotedMonoVertexStatus, promotedChildDef, c)
	if err != nil {
		return true, err
	}
...
func (r *MonoVertexRolloutReconciler) ProcessPromotedChildPostUpgrade(
	...
	performedScaling, err := scaleMonoVertexToDesiredValues(ctx, monoVertexRollout.Status.ProgressiveStatus.PromotedMonoVertexStatus, promotedChildDef, c)
	if err != nil {
		return true, err
	}
...
func scaleMonoVertexToDesiredValues(
	...
	if _, exists := promotedMVStatus.ScaleValues[promotedChildDef.GetName()]; !exists {
	...
	patchJson := fmt.Sprintf(
		...
		promotedMVStatus.ScaleValues[promotedChildDef.GetName()].DesiredMin,
		promotedMVStatus.ScaleValues[promotedChildDef.GetName()].DesiredMax,
	)
