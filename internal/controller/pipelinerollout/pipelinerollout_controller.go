...
func (r *PipelineRolloutReconciler) reconcile(
	...
	defer func() {
		...
		if pipelineRollout.DeletionTimestamp.IsZero() {
			...
			r.customMetrics.SetPipelineRolloutHealth(pipelineRollout.Namespace, pipelineRollout.Name, string(pipelineRollout.Status.Phase), apiv1.HasFailureCondition(pipelineRollout.Status.Conditions))
		}
	}()
